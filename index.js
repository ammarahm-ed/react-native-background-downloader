...
RNBackgroundDownloaderEmitter.addListener("downloadAdded", event => {
  let task = tasksMap.get(event.id);
  if (task) {
    task._onAdded(event.expectedBytes);
  }
});
RNBackgroundDownloaderEmitter.addListener("downloadPaused", event => {
  let task = tasksMap.get(event.id);
  if (task) {
    task._onPaused();
  }
});
RNBackgroundDownloaderEmitter.addListener("downloadResumed", event => {
  let task = tasksMap.get(event.id);
  if (task) {
    task._onResumed();
  }
...
RNBackgroundDownloaderEmitter.addListener("downloadQueued", event => {
  let task = tasksMap.get(event.id);
  if (task) {
    task._onQueued(event.waitingForNetwork);
  }
});
RNBackgroundDownloaderEmitter.addListener("waiting_for_network", event => {
  let task = tasksMap.get(event.id);
  if (task) {
    task._onWaitingForNetwork();
  }
});
...
export function checkForExistingDownloads() {
  return RNBackgroundDownloader.checkForExistingDownloads().then(foundTasks => {
    return foundTasks
      .map(taskInfo => {
...
export function fileExists(path, cb) {
  RNBackgroundDownloader.fileExists(path, cb);
}
